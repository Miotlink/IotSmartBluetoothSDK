...
import com.miotlink.ble.service.AwsSmartConfigImpl;
...
public class BlueISmartImpl extends BleWriteCallback<BleModelDevice> implements ISmart {
    ...
    public BleModelDevice  getBleModelDevice(String macCode) {
        ...
        return hasAddDeviceStores.getBleModelDevice(macCode);
    }
    @Override
    public BleModelDevice  getScanBleModelDevice(String macCode) {
        BleModelDevice bleDevice=null;
        if (ble!=null){
            bleDevice = ble.getBleDevice(macCode);
        }
        if (bleDevice==null){
            bleDevice= bluetoothDeviceStore.getBleModelDevice(macCode);
        }
        return bleDevice;
    }
...
//    }
    ...
    @Override
   public BleModelDevice getScanBindDevice(String macAddress){
        ...
        BleModelDevice bleDevice = null;
        if (bleDevice==null){
           return hasAddDeviceStores.getBleModelDevice(macAddress);
        }
        return null;
    }
...
//                Log.e("bleScan", hexScanRecord);
    ...
    @Override
    public void onConnectByMac(String macCode, ILinkConnectCallback mLinkConnectCallback) throws Exception {
        if (ble == null) {
            ble = Ble.getInstance();
        }
        this.mLinkConnectCallback=mLinkConnectCallback;
        ble.connect(macCode, connectCallback);
    ...
    public void onAwsStartSmartConfig(String macCode, String  awsNetworkInfo, int delayMillis, ILinkSmartConfigListener mILinkSmartConfigListener1) throws Exception {
        ...
        ble.connect(bleModelDevice, new AwsSmartConfigImpl(ble,smartMac, awsNetworkInfo) {
            @Override
            protected void onReceiver(BleModelDevice device,String macCode, int awsErrorCode, String message, String data)throws Exception {
                ...
                if (awsErrorCode==65){
                    ...
                    errorCode=7015;
                    if (mILinkSmartConfigListener!=null)
                    mILinkSmartConfigListener.onLinkSmartConfigListener(errorCode, errorMessage, macCode);
                    return;
                }
...
//                            Log.e("error", decode.toString());
    ...
    public void disConnected(BleModelDevice modelDevice) throws Exception {
        ...
        if (modelDevice!=null&&ble!=null){
            Log.e("disConnected", "断开连接");
            ble.disconnect(modelDevice);
            bluetoothDeviceStore.clear();
        }
    }
