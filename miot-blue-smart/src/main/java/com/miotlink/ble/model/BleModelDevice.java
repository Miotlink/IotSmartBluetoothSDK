...
public final class BleModelDevice extends BleDevice implements Parcelable {
    ...
    public void setScanRecord(ScanRecord scanRecord) {
        if (scanRecord!=null){
            ...
            try {
                byte[] bytes = scanRecord.getManufacturerSpecificData().get(26470);
                String scanRecordStr="";
                if (bytes!=null){
                    scanRecordStr = HexUtil.encodeHexStr(bytes);
                }
                if (TextUtils.isEmpty(scanRecordStr)&&scanRecordStr.startsWith("6667")
                        ||bytes!=null&&bytes.length>18){
                    if (!TextUtils.isEmpty(getBleName())){
                        deviceName=getBleName();
                    }
                    mCode=7;
                    mVersion=(int)bytes[2];
                    mark=(int)bytes[3];
                    kindId= BlueTools.BytesToInt32BE(bytes,5);
                    modelId=BlueTools.BytesToInt32BE(bytes,9);
                    macAddress="";
                ...
                if (mScanRecord!=null&&mScanRecord.length>20){
                    ...
                    if (bytesMac!=null){
                        if (bytesMac[5]==0x66&&bytesMac[6]==0x67){
                            ...
                            macAddress="";
                            macAddress+=BlueTools.byteToHex(bytesMac[25])+":";
                            macAddress+=BlueTools.byteToHex(bytesMac[26])+":";
                            macAddress+=BlueTools.byteToHex(bytesMac[27])+":";
                            macAddress+=BlueTools.byteToHex(bytesMac[28])+":";
                            macAddress+=BlueTools.byteToHex(bytesMac[29])+":";
                            macAddress+=(BlueTools.byteToHex(bytesMac[30])+"");
                            macAddress=macAddress.toUpperCase();
                            setMacAddress(macAddress);
                        }
                    }
